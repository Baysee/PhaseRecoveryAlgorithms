...
if 1/(2*TargetResolution)>fSpecExp(end)
    ...
    [a,nZeros]=find(f>-abs(fSpecGHz(end)*1e9),1);
...
%     if iIter==1; plotIter=1;end
    ...
    [xt,Si,diCPlot,diRPlot]=phaseRecovLoop(nIncsInterp,windowCentersInterp,lent,winInterp,winLen,t,dt,xt0,tspgm,fspgm,spgm,SUT,analysisWin,Fs,maxIteration,{'Inconsistency','frogE'},plotIter,filtm,fMaxStftFilter,plotFilt,f);
...
alldiCs(icl, inptWin, ibwIRF, iInter)=diCPlot(end);
